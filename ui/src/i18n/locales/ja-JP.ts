...
export const ja_JP: TranslationMap = {
  ...
  channels: {
    ...
    whatsapp: {
      ...
      loggedOut: "ログアウトしました。",
      logoutNotCleared:
        "保存された WhatsApp セッションは消去されませんでした。すでに存在しないか、認証ディレクトリを手動でクリーンアップする必要がある可能性があります。",
    },
  ...
  custodian: {
    ...
    history: {
      ...
      opaqueChange: "表示可能な構成パスの変更なしに、書式またはコメントが変更されました。",
      sources: {
    ...
    nudge: {
      configReload: "設定の再読み込みが停止しました — 何が起きたか尋ねてください",
  ...
  mcpServers: {
    ...
    adminRequired: "MCPサーバーの変更にはoperator.adminアクセスが必要です。",
    enable: "有効化：",
    disable: "無効化",
  ...
  labsPage: {
    intro:
      "Labsには、リリース間で変更、破損、または消滅する可能性のある実験的な機能が含まれています。",
    sectionTitle: "実験的な機能",
    ...
    swarm: {
      title: "Swarm",
      description: "Code Modeがサブエージェントのグループを並行して調整できるようにします。",
  ...
  appsPage: {
    ...
    cards: {
      ...
      android: {
        title: "Android",
        desc: "Android スマートフォンを完全な OpenClaw ノードとして — チャット、カメラ、Canvas を利用できます。",
      },
      appleWatch: {
        title: "Apple Watch",
        desc: "手元でチャットをひと目で確認し、素早く返信できます。",
      },
      wearOs: {
        title: "Wear OS",
        desc: "Android コンパニオンが OpenClaw を時計に拡張します。",
      },
      macos: {
        title: "macOS",
  ...
  profilePage: {
    ...
    identity: {
      ...
      displayName: "表示名",
      displayNameDescription: "このゲートウェイを使用する他のユーザーに表示されます。",
      linkedEmails: "リンク済みのメール",
  ...
  board: {
    ...
    actionInProgress: "別のダッシュボード変更をまだ保存中です。",
    announcement: {
      moved: "{title} を移動しました。",
    ...
    widget: {
      ...
      rejectedDetail:
        "このウィジェットは、削除または置き換えられるまで非アクティブのままになります。",
  ...
  chat: {
    ...
    composer: {
      ...
      holdToRecordSettingDescription:
        "作成欄のマイクボタンを押し続けて話し、離すと送信せずにテキストを挿入します。",
      dictationAudioUnsupported: "Gateway がサポートされていない音声入力形式を返しました。",
      dictationBrowserAudioUnsupported: "このブラウザでは 8 kHz で音声入力を取得できません。",
      dictationConnecting: "音声入力を開始しています…",
      dictationDisconnected: "Gateway が切断されたため、音声入力を停止しました。",
      dictationFailed: "音声入力に失敗しました。",
      dictationFinalizing: "音声入力を完了しています…",
      dictationFinalizationTimedOut: "最後の部分文字起こしを確定する前に音声入力が停止しました。",
      dictationProviderUnavailable: "音声入力用の文字起こしプロバイダーが設定されていません。",
      dictationRecording: "録音中 {elapsed}",
      dictationReleaseToInsert: "放すと音声入力を挿入します",
      ...
      selectedCameraUnavailable:
        "選択したカメラは利用できません。別のカメラまたはシステム既定を選択してください。",
